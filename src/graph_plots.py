...
def plot_graph(x, y, xlabel, ylabel, title, window_size=30):
    ...
    for i in range(window_size//2, len(y) - window_size//2):
        smoothed_y.append(
            np.mean(y[i - window_size//2: i + window_size//2 + 1]))
    plt.plot(x[window_size//2: len(y) - window_size//2],
             smoothed_y,  color='red', alpha=0.7)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
    plt.tight_layout()
...
def plot_comparison_graph(x1, y1, x2, y2, xlabel, ylabel, t1, t2, title, sharey, window_size=20):
    ...
    for i in range(window_size//2, len(y1) - window_size//2):
        smoothed_y1.append(
            np.mean(y1[i - window_size//2: i + window_size//2 + 1]))
    for i in range(window_size//2, len(y2) - window_size//2):
        smoothed_y2.append(
            np.mean(y2[i - window_size//2: i + window_size//2 + 1]))
    ...
    axs[0].set_title(t1)
    axs[0].plot(x1[window_size//2: len(y1) - window_size//2],
                smoothed_y1,  color='red', alpha=0.7)
    ...
    axs[1].set_title(t2)
    axs[1].plot(x2[window_size//2: len(y2) - window_size//2],
                smoothed_y2,  color='red', alpha=0.7)
...
if __name__ == "main":
    ...
    sns.heatmap(max, cbar=True, square=True, annot=ind,
